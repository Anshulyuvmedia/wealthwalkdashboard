...
const EditPlan: React.FC = () => {
    ...
    React.useEffect(() => {
        const fetchPlan = async () => {
            try {
                ...
                // Merge fetched features with predefined features to ensure all are included
                const mergedFeatures = predefinedFeatures.map((predefined) => {
                    const existing = plan.fetures.find((f: { title: string }) => f.title === predefined.title);
                    return existing ? { ...predefined, enabled: existing.enabled } : predefined;
                });
                setFormData({
                    planName: plan.planName,
                    Duration: plan.Duration,
                    durationValue: plan.durationValue,
                    fetures: mergedFeatures,
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await apiService.updatePlan(id!, planData);
